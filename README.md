...
MVP уже работает и показан преподавателю. Дальше бота можно развивать по шагам:

1. **Счёт и статистика** — начислять очки за верный ответ, хранить прогресс в Redis, показывать серию и процент угадываний.
2. **Категории и туры** — фильтровать вопросы по теме и сложности, собирать подборки.
3. **Таймер на вопрос** — ограничивать время ответа и автоматически вскрывать ответ по таймауту.
4. **Рейтинг и лидерборд** — таблица лидеров среди игроков.
5. **Прогрессивные подсказки** — открывать всё больше букв ответа по мере повторных неудач.
6. **Мультиязычность** — локализация пользовательских строк.
7. **Тесты и CI** — unit-тесты на `normalize_answer`/`evaluate_answer`, линтинг и проверка типов в пайплайне.
8. **Контейнеризация** — `Dockerfile` и `docker-compose` с Redis для запуска одной командой.
